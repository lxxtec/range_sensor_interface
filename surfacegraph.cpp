    ...
    : m_graph(surface)
{
    ...
    //! [2]
    QImage heightMapImage(":/maps/aaa");
...
void SurfaceGraph::filldata(){
    float stepX = (sampleMax - sampleMin) / float(sampleCountX - 1); // X 轴步进
    float stepZ = (sampleMax - sampleMin) / float(sampleCountZ - 1); // Z 轴步进
    QSurfaceDataArray *dataArray = new QSurfaceDataArray;
    dataArray->reserve(sampleCountZ); // Z轴的数目
...
void SurfaceGraph::enableSqrtSinModel(bool enable)
{
    if (enable) {
        ...
        //! [8]
        //!
        QImage img = m_graph->renderToImage(0,QSize(1920,1080));
        img.save("sqrt_sin.jpg","jpg");
    }
}
